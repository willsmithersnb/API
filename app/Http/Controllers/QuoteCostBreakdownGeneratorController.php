...
class QuoteCostBreakdownGeneratorController extends Controller
{
    protected $total_quote_size;
    ...
    protected $total_quote_size_styles = [
        'font' => [
            'bold' => true,
            'size' => 10,
            'name' => 'Arial'
    ...
    public function calculateTotalQuoteSize($packaging_options)
    {
        foreach ($packaging_options as $packaging_option) {
            $this->total_quote_size += $packaging_option->quantity * $packaging_option->fill_amount;
        }
    }
     ...
     */
    public function __invoke(Request $request, int $quote_id)
    {
        // file name.
        $template_name = 'quote_' . $quote_id . '_cost_breakdown';

        // fetching data
        $item_list = Quote::unScoped()->findOrFail($quote_id)->itemList()->unScoped()->first();
        ...
        $this->calculateTotalQuoteSize($item_packaging_options);
        ...
        $spreadsheet->getActiveSheet()->setTitle('Quote Price Breakdown');
        ...
        // fill cell with data
        $sheet->getStyle('I2:J2')->applyFromArray($this->total_quote_size_styles);
        $sheet->setCellValue('I2', 'Total Quote Size');
        $sheet->setCellValue('J2', UnitHelper::convertTo($this->total_quote_size, 'pL', 'L'));
        ...
        $this->row_count += 3;
        if (($item_list->discount != null) || ($item_list->adjustments != null)) {
            $this->row_count += 2;
            ...
            $sheet->setCellValue('A' . $this->row_count, 'Discounts & Adjustments');
            $sheet->getStyle('H' . $this->row_count)->applyFromArray($this->discount_price_styles);
            $sheet->setCellValue('H' . $this->row_count, ($item_list->adjustments / 100) - ($item_list->discount / 100));
        }
    ...
    public function writeIngredientsToSheet($sheet, $formula_ingredients)
    {
        ...
        $sheet->mergeCells('A' . $this->row_count . ':G' . $this->row_count);
        $sheet->getStyle('A' . $this->row_count . ':G' . $this->row_count)->applyFromArray($this->total_values_text_styles);
        $sheet->setCellValue('A' . $this->row_count, 'Final Ingredients Price (Total Ingredients Price * Total Quote Size)');
        ...
        foreach ($item_pricing_addon_tiers as $item_pricing_addon_tier) {
            ...
            switch ($pricing_addon->conditional_variable) {
                ...
                case ('num_ingredients_changed'):
                    $quantity = $item_packaging_option->quantity;
                    break;
            }
    ...
    public function createAddonSheet($spreadsheet)
    {
        ...
        foreach ($pricing_addon_tiers as $pricing_addon_tier) {
            ...
            $sheet2->setCellValue('B' . $this->row_count, 'Total Quote Size > ' . $pricing_addon_tier->condition_greater_than);
