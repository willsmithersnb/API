...
try {
$env_prefix = isset($env) ? strtoupper($env) : "DEV";
$dotenv->load();
$dotenv->required(['DEPLOY_'.$env_prefix.'_SERVER', 'DEPLOY_REPOSITORY', 'DEPLOY_PATH'])->notEmpty();
} catch ( Exception $e ) {
echo $e->getMessage();
exit;
}

$server = $_ENV['DEPLOY_'.$env_prefix.'_SERVER'] ?? null;
...
$branch =( $env == "prod") ? "production" : (isset($branch) ? $branch : "development");
$path = rtrim($path, '/');
$release = $path.'/'.$date;
...
echo {{"[Release URI]: ".$release}}
cd {{ $path }}
echo "Deployment ({{ $date }}) started"
git clone {{ $repo }} --branch={{ $branch }} --depth=1 -q {{ $release }}
echo "Repository cloned"
cp -rf {{ $release }}/storage/* {{ $path }}/storage
echo "Copied git tracked storage files"
@endtask
